    ...
    return (
        <View style={{...styles.header, backgroundColor: colors.card, borderColor: colors.border}}>  
            <TouchableOpacity style={{...styles.left, left: leftChevron ? 10 : 15}} onPress={leftOnPress}>
                {leftChevron ? <Feather name="chevron-left" size={16} color={colors.interactive}/> : <View/>}
            ...
            <View style={{...styles.right, right: rightChevrom ? 10 : 15}}>
                {plus 
                    ?<TouchableOpacity onPress={plusOnPress}>
                        <Feather name="plus" size={14} color={colors.interactive} style={{marginRight: 15}}/> 
                    </TouchableOpacity>
                    : <View/>}
                <TouchableOpacity onPress={rightOnPress}>
                    <Text style={{...styles.text, color: colors.interactive}}>{rightText}</Text>
                    {rightChevrom ? <Feather name="chevron-right" size={16} color={colors.interactive}/> : <View/>}
                </TouchableOpacity>
            </View>
        </View>
    );
