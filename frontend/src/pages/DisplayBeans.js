...
const DisplayBeans = ({ route, navigation }) => {
    ...
    const onFavorite = (id) => {
        let val = 0;
        brews.forEach((brew) => {
            if (brew.id === id) val = brew.favorite;
        });
        ...
        setBrews(
            brews.map((brew) =>
                brew.id === id
                    ? { ...brew, favorite: brew.favorite === 0 ? 1 : 0 }
                    : brew
            )
        );
    ...
    useEffect(() => {
        ...
        if (beans.flavor_notes === "") setFlavorNotes([]);
    }, [beans]);
    ...
    return (
        <View style={styles.container}>
                    ...
                    >
                        <View style={styles.row}>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => favoriteBeans()}
                            >
                                <FontAwesome
                                    name={beans.favorite ? "heart" : "heart-o"}
                                    size={25}
                                    color={
                                        beans.favorite
                                            ? "#a00"
            ...
            <View
                ref={brewRef}
                style={{ position: "absolute", width: width, left: width }}
            >
                {selected.id && (
                    <Brew
                        brew={selected}
            ...
            {showModal && (
                ...
                >
                    ...
                    <TouchableOpacity
                        onPress={() =>
                            navigation.navigate("ReviewRecipe", {
                                brew: selected,
                            })
                        }
                    >
                    ...
                    <TouchableOpacity
                        onPress={() => {
                            Clipboard.setString(
                                toBrewString(selected, user_preferences.grinder)
                            );
                            Alert.alert(
                                "Copied",
                                "Copied brew recipe to clipboard.",
                                [
                                    {
                                        text: "OK",
