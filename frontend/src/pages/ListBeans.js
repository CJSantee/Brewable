...
const Beans = ({ beans, onLongPress, navigation }) => {
    ...
    return (
        <TouchableOpacity
            onPress={() =>
                navigation.navigate("DisplayBeans", {
                    beans_id: beans.id,
                    parent: "ListBeans",
                })
            }
...
const ListBeans = ({ navigation }) => {
    ...
    function searchFilter(item, query) {
        ...
        if (item.roaster.toLowerCase().includes(query)) return true;
    ...
    function handleSearch(newSearchQuery) {
        ...
        setSearchResults(
            beans.filter((item) => searchFilter(item, newSearchQuery))
        );
    }
    ...
    const onFavorite = (id) => {
        let val = 0;
        beans.forEach((beans) => {
            if (beans.id === id) val = beans.favorite;
        });
        // Update brew in database
        db.transaction(
            (tx) => {
                tx.executeSql("UPDATE beans SET favorite = ? WHERE id = ?;", [
                    val === 0 ? 1 : 0,
                    id,
                ]);
            },
    ...
    return (
        ...
        >
            ...
            {btmModal && (
                <FullScreenModal
                    colors={colors}
                    close={() => setBtmModal(false)}
                >
                    <TouchableOpacity
                        onPress={() =>
                            navigation.navigate("EditBeans", {
                                beans_id: selected.id,
                                flavor_notes: selected.flavor_notes,
                            })
                        }
                    >
                    ...
                    <TouchableOpacity
                        onPress={() =>
                            navigation.navigate("NewBrew", {
                                beans_id: selected.id,
                                roaster: selected.roaster,
                                name: selected.name,
                            })
                        }
                    >
                    ...
                    <TouchableOpacity
                        onPress={() => {
                            Clipboard.setString(toBeansString(selected));
                            Alert.alert(
                                "Copied",
                                `"${selected.roaster} - ${selected.name}" copied to clipboard`,
                                [
                                    {
                                        text: "OK",
                                        onPress: () => setBtmModal(false),
                                    },
                                ]
                            );
                        }}
