...
const BrewMethods = ({ route, navigation }) => {
    ...
    const newBrewMethodPrompt = () => {
        Alert.prompt("Add Brew Method", "Enter new brew method name", [
            {
                text: "Cancel",
                onPress: () => {},
            },
        ...
        db.transaction(
            (tx) => {
                selected.forEach((value) =>
                    tx.executeSql(
                        "DELETE FROM brew_methods WHERE method = ?;",
                        [value]
                    )
                );
            },
    ...
    const updateBrew = (_brew_method) => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "UPDATE brews SET brew_method = ? WHERE id = ?;",
                    [_brew_method, brew_id]
                );
            },
            (e) => console.log(e),
    ...
    return (
        <View style={{ height: "100%", width: "100%" }}>
            <Header
                ...
                plus={true}
                plusOnPress={newBrewMethodPrompt}
            />
            <FlatList
                data={methods}
                renderItem={({ item }) => (
                    <RowItem
                        ...
                        onPress={
                            parent === "EditBrew"
                                ? () => updateBrew(item.method)
                                : () =>
                                      navigation.navigate(parent, {
                                          brew_method: item.method,
                                      })
                        }
                        showSelect={editing}
