...
const SelectFlavors = ({ route, navigation }) => {
    ...
    function handleSearch(newSearchQuery) {
        ...
        setSearchResults(
            flavors.filter((item) => searchFilter(item, newSearchQuery))
        );
    }
    ...
    const newFlavorPrompt = () => {
        Alert.prompt("Add Flavor", "Enter new flavor note", [
            {
                text: "Cancel",
                onPress: () => {},
            },
        ...
        db.transaction(
            (tx) => {
                selected.forEach((value) =>
                    tx.executeSql("DELETE FROM flavors WHERE flavor = ?;", [
                        value,
                    ])
                );
            },
            ...
            () => {
                selected.forEach((value) => {
                    if (picked.has(value))
                        // If values being deleted are picked, unpick them -> Still not working
                        togglePicked(value);
                });
    ...
    const updateBeans = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "UPDATE beans SET flavor_notes = ? WHERE id = ?;",
                    [flavorNotes(), beans_id]
                );
            },
            (e) => console.log(e),
    ...
    return (
        <View style={{ height: "100%", width: "100%" }}>
            <Header
                ...
                rightText={
                    editing || parent === "EditBeans" || parent === "NewBeans"
                        ? "Done"
                        : "Edit"
                }
                leftOnPress={
                    editing ? deleteSelected : () => navigation.goBack()
                }
                leftChevron={editing ? false : true}
                rightOnPress={rightOnPress}
                plus={true}
                plusOnPress={newFlavorPrompt}
            />
            <SearchBar
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <RowItem
                        ...
                        text=''
                        onPress={() => {
                            if (parent == "NewBeans" || parent == "EditBeans")
                                togglePicked(item.flavor);
                        }}
                        showSelect={editing}
                        selected={selected.has(item.flavor)}
                        toggleSelect={(value) => toggleSelected(value)}
                    >
                        {picked.has(item.flavor) && (
                            <Feather
                                name='check'
                                size={20}
